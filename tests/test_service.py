...
def test_validate_patches(service):
    ...
    # Valid patches
    patches = [
        EditPatch(start=1, end=2, contents="content1", range_hash=""),
        EditPatch(start=3, end=4, contents="content2", range_hash=""),
    ]
    assert service.validate_patches(patches, 5) is True

    # Overlapping patches
    patches = [
        EditPatch(start=1, end=3, contents="content1", range_hash=""),
        EditPatch(start=2, end=4, contents="content2", range_hash=""),
    ]
    assert service.validate_patches(patches, 5) is False

    # Out of bounds patches
    patches = [EditPatch(start=1, end=10, contents="content1", range_hash="")]
    assert service.validate_patches(patches, 5) is False
